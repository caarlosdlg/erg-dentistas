...
const Citas = () => {
  ...
  // Función para cargar pacientes OPTIMIZADA con endpoint específico para dropdown
  const loadPatients = async () => {
    console.log('🔍 Citas: Cargando pacientes para dropdown...');
    ...
    try {
      ...
      // ✅ CAMBIO APLICADO: Usando getPatientsForDropdown() en lugar de getPatients()
      const data = await apiService.getPatientsForDropdown();
      const patientsData = data.results || data || [];
      setPatients(patientsData);
      console.log('✅ Citas: Pacientes cargados para dropdown (optimizado):', patientsData.length);
    } catch (error) {
        ...
        <div className="mt-3 flex gap-4 text-xs">
          {patients.length > 0 && (
            <div className="text-green-700">
              ✅ Pacientes conectados a BD (endpoint optimizado)
            </div>
          )}
