...
const CitasCompleto = () => {
  console.log('🔍 CitasCompleto: Componente iniciando...');
  ...
  const loadPatients = async () => {
    console.log('🔍 CitasCompleto: Cargando pacientes...');
    ...
    try {
      ...
      console.log('✅ CitasCompleto: Pacientes cargados:', patientsData.length);
    } catch (error) {
      console.error('❌ CitasCompleto: Error al cargar pacientes:', error);
      setPatients([]);
    } finally {
  ...
  const loadAppointments = async () => {
    console.log('🔍 CitasCompleto: Cargando citas...');
    ...
    try {
      ...
      if (citasFromDB.length > 0) {
        ...
        console.log('✅ CitasCompleto: Citas cargadas desde BD:', transformedAppointments.length);
      } else {
        setAppointments(mockAppointments);
        console.log('ℹ️ CitasCompleto: Usando datos mock');
      }
    } catch (error) {
      console.error('❌ CitasCompleto: Error al cargar citas:', error);
      setAppointments(mockAppointments);
      console.log('⚠️ CitasCompleto: Usando datos mock por error');
    } finally {
  ...
  useEffect(() => {
    console.log('🔍 CitasCompleto: useEffect ejecutándose...');
    
    const initializeData = async () => {
      try {
        await loadPatients();
        await loadAppointments();
        console.log('✅ CitasCompleto: Inicialización completada');
      } catch (error) {
        console.error('❌ CitasCompleto: Error en inicialización:', error);
      }
    };
    ...
    if (selectedPatient && newAppointment.date && newAppointment.time) {
      try {
        const citaData = {
          ...
          motivo_consulta: newAppointment.service,
      ...
      alert('Por favor complete todos los campos obligatorios (Paciente, Fecha y Hora)');
    }
  };
  ...
  console.log('✅ CitasCompleto: Renderizando componente...');
